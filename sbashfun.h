...
void print_sbash_info() {
  ...
  printf(
      "Description: A secure version of Bash for Organizations, enterprises, "
      "and \neducational institutions seeking to secure their systems, by "
...
int is_special_ch_in_command(char* str, int len) {
  ...
  for (int i = 0; i < len; i++) {
    if (str[i] == '!' || str[i] == '@' || str[i] == '#' || str[i] == '$' ||
        ...
        str[i] == '<' || str[i] == '?') {
      printf(
          "sbash: \033[1;37mspecial symbols are not allowed for security "
          "reasons!\033[0m\n");
      flag = 1;
      break;
    }
